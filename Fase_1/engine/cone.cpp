...
void drawCone(float radius, float height, const int slices, int stacks, char* filename) {
	...
	//glBegin(GL_TRIANGLES);
		...
		for (int i = 0; i < slices; i++) {
			...
			//base de baixo
			fprintf(f, "%f; %f; %f\n", radius * sin(angle * (i + 1)), 0, radius * cos(angle * (i + 1)));
			fprintf(f, "%f; %f; %f\n", radius * sin(angle * i), 0, radius * cos(angle * i));
			fprintf(f, "%f; %f; %f\n", 0, 0, 0);
...
void drawCone(const char* filename) {
	//ESTA FUNÇÃO DESENHA A FIGURA DO CONE LENDO OS VÉRTICES QUE ESTÃO CONTIDOS NO FICHEIRO cone.3d
