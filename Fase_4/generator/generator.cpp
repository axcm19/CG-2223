...
// versão que gera o ficheiro 3d com coordenadas do ponto, coordenadas de textura e normal
// cada linha do ficheiro 3d corresponde a um vértice V com V_x, V_y, V_z, V_Texture_x, V_Texture_y, V_Normal_x,  V_Normal_y, V_Normal_z
void generateSphere_Texture_Normal(float radius, const int slices, int stacks, char* filename) {
	// ESTA FUNÇÃO SERVE APENAS PARA DEFINIR OS VÉRTICES QUE VÃO COMPOR A FIGURA DA ESFERA E ESCREVE-LOS NO FICHEIRO sphere.3d
	...
	for (int i = 0; i < indices.size(); i += 3) {
		...
		fprintf(f, "%f; %f; %f; %f; %f; %f; %f; %f\n",
			vertices[index1 * 3], vertices[index1 * 3 + 1], vertices[index1 * 3 + 2],	// ---> coordenadas x,y,z
			texCoords[index1 * 2], texCoords[index1 * 2 + 1],							// ---> coordenadas de textura
			normals[index1 * 3], normals[index1 * 3 + 1], normals[index1 * 3 + 2]);		// ---> normal do vértice

		fprintf(f, "%f; %f; %f; %f; %f; %f; %f; %f\n",
			vertices[index2 * 3], vertices[index2 * 3 + 1], vertices[index2 * 3 + 2],
			texCoords[index2 * 2], texCoords[index2 * 2 + 1],
			normals[index2 * 3], normals[index2 * 3 + 1], normals[index2 * 3 + 2]);

		fprintf(f, "%f; %f; %f; %f; %f; %f; %f; %f\n",
			vertices[index3 * 3], vertices[index3 * 3 + 1], vertices[index3 * 3 + 2],
			texCoords[index3 * 2], texCoords[index3 * 2 + 1],
			normals[index3 * 3], normals[index3 * 3 + 1], normals[index3 * 3 + 2]);
	}
	fclose(f);
}
...
void generateCone(float radius, float height, int slices, int stacks, char* filename) {
	...
	for (int i = 0; i < slices; i++) {
		...
		//um triangulo para cada slice i
		fprintf(f, "%f; %f; %f\n", 0.0f, height, 0.0f);
		fprintf(f, "%f; %f; %f\n", radius * sin(angle * i), 0.0f, radius * cos(angle * i));
		fprintf(f, "%f; %f; %f\n", radius * sin(angle * (i + 1)), 0.0f, radius * cos(angle * (i + 1)));
	}
...
// versão que gera o ficheiro 3d com coordenadas do ponto, coordenadas de textura e normal
// cada linha do ficheiro 3d corresponde a um vértice V com V_x, V_y, V_z, V_Texture_x, V_Texture_y, V_Normal_x,  V_Normal_y, V_Normal_z
void generateCone_Texture_Normal(float radius, float height, const int slices, int stacks, char* filename) {
	// ESTA FUNÇÃO SERVE APENAS PARA DEFINIR OS VÉRTICES QUE VÃO COMPOR A FIGURA DO CONE E ESCREVE-LOS NO FICHEIRO cone.3d
	...
	for (int i = 0; i < slices; i++) {
		...
		// Base de baixo
		fprintf(f, "%f; %f; %f; %f; %f; %f; %f; %f\n",
			radius * sin(angle * (i + 1)), 0.0f, radius * cos(angle * (i + 1)),			// ---> coordenadas x,y,z
			(angle * (i + 1)) / (2 * M_PI), 0.0f,										// ---> coordenadas de textura
			0.0f, -1.0f, 0.0f);															// ---> normal do vértice
		...
		fprintf(f, "%f; %f; %f; %f; %f; %f; %f; %f\n",
			radius * sin(angle * (i + 1)), 0.0f, radius * cos(angle * (i + 1)),
			(angle * (i + 1)) / (2 * M_PI), 0.0f,
			sin(angle * (i + 1)), 0.0f, cos(angle * (i + 1)));
	}
	std::fclose(f);
}
...
// versão que gera o ficheiro 3d com coordenadas do ponto, coordenadas de textura e normal
// cada linha do ficheiro 3d corresponde a um vértice V com V_x, V_y, V_z, V_Texture_x, V_Texture_y, V_Normal_x,  V_Normal_y, V_Normal_z
void generateBox_Texture_Normal(float length, int divisions, char* filename) {
	float h = length / 2;
	float l = length / divisions;
	float d, t;
	...
	for (d = 0; d < divisions * l; d += l) {
		for (t = 0; t < divisions * l; t += l) {
			...
			fprintf(f, "%f; %f; %f; %f; %f; %f; %f; %f\n",
				-h + t, -h, -h + d,				// ---> coordenadas x,y,z
				t / length, d / length,			// ---> coordenadas de textura
				0.0f, -1.0f, 0.0f);				// ---> normal do vértice
...
// versão que gera o ficheiro 3d com coordenadas do ponto, coordenadas de textura e normal
// cada linha do ficheiro 3d corresponde a um vértice V com V_x, V_y, V_z, V_Texture_x, V_Texture_y, V_Normal_x,  V_Normal_y, V_Normal_z
void generatePlane_Texture_Normal(float length, int divisions, const char* filename) {
	float h = length / 2;
	float l = length / divisions;
	float d, t;
	...
	int vertexCount = divisions * divisions * 6;

	for (d = 0; d < divisions * l; d += l) {
		for (t = 0; t < divisions * l; t += l) {
			// Triângulo 1
			fprintf(f, "%f; %f; %f; %f; %f; %f; %f; %f\n",
				-h + t, 0.0f, -h + d,			// ---> coordenadas x,y,z
				t / length, d / length,			// ---> coordenadas de textura
				0.0f, 1.0f, 0.0f);				// ---> normal do vértice
...
int main(int argc, char **argv) {
    ...
    if (argc == 1) {
        ...
        if (strcmp(solid, "plane") == 0) {
            ...
            generatePlane_Texture_Normal(length, num_div, filename);
            return 0;
        }
            ...
            float num_div = (float)atoi(argv[3]);
            char* filename = argv[4];
            generateBox_Texture_Normal(length, num_div, filename);
            return 0;
        }
            ...
            generateSphere_Texture_Normal(radius, slices, stacks, filename);
            return 0;
        }
            ...
            generateCone_Texture_Normal(radius, height, slices, stacks, filename);
            return 0;
        }
            ...
            generateBezier(file1, file2, tesselation);	//./generator bezier "teapot.patch" "teapot.3d"
            return 0;
        }
